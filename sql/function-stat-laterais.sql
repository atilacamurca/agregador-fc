...
CREATE OR REPLACE FUNCTION stat_laterais(_rodada_id integer, _ano integer, _clube_id integer) 
RETURNS json AS
$$
    SELECT row_to_json(t) AS json FROM (
        select coalesce(sum(rb), 0) as sum_rb, coalesce(sum(g), 0) as sum_g,
        coalesce(sum(a), 0) as sum_a, coalesce(sum(fs), 0) as sum_fs,
        coalesce(sum(pe), 0) as sum_pe, coalesce(sum(ff), 0) as sum_ff,
